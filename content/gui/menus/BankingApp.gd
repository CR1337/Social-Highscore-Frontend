...
onready var _balance_label = $Background/Margin/VBox/HBox/BalanceLabel
onready var _transaction_history_label = $Background/Margin/VBox/TransactionHistoryLabel
...
func _display_balance():
	_balance_label.text = str(GameStateController.money) + " ¥"

func _display_transactions():
	_transaction_history_label.clear()
	for transaction in _transactions:
		var color = "green"
		if transaction[0] < 0:
			color = "red"
		_transaction_history_label.append_bbcode(
			"\n\n[color=" + color + "]" + transaction[1] + "[right]" + str(transaction[0]) + " ¥[/right][/color]"
		)
	_transaction_history_label.scroll_to_line(_transaction_history_label.get_line_count() - 1)
